...
function isActivityToday(activity: { startDate?: string; dueDate?: string }) {
  return isToday(activity.startDate) || isToday(activity.dueDate);
}

function isActivityWithinNextDays(activity: { startDate?: string; dueDate?: string }, days: number) {
  return isWithinNextDays(activity.startDate, days) || isWithinNextDays(activity.dueDate, days);
}
...
export function DashboardPage() {
  ...
  const todayActivities = activities.filter(isActivityToday);
  const weekActivities = activities.filter((activity) => isActivityWithinNextDays(activity, 7));
